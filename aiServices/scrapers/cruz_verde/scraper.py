...
BASE_URL = "https://www.cruzverde.com.co"
class CruzVerdeScraper(BaseScraper):
    STORE_NAME = "cruz_verde"
    REQUIRES_JS = True
    async def search(self, query: str, max_results: int = 10) -> list[Product]:
        url = f"{BASE_URL}/buscador?query={query}"
        ...
        products = []
        for card in soup.select("[class*='product'], [class*='item-product']")[:max_results]:
            try:
